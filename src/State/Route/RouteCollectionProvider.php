 ...
 *
 * Supports an optional `?driver={id}` query parameter to list a
 * specific driver's routes (useful for parents selecting a route
 * during payment with per-route pricing).
 *
 * - ROLE_SCHOOL_ADMIN / ROLE_SUPER_ADMIN: all routes (filterable by ?driver=)
 * - ROLE_DRIVER: only routes assigned to the authenticated driver
 * - ROLE_PARENT: ?driver= → driver's routes; otherwise routes with their students' stops
 * - Others: empty collection
 *
 * School scoping is handled automatically by the SchoolFilter on Route.
 *
