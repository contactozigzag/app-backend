...
All recurring jobs live in a single aggregated provider: `src/Scheduler/AppScheduleProvider.php` (`#[AsSchedule('default')]`). The messenger worker consumes `scheduler_default` alongside `async`.

**To add a new recurring job: add another `RecurringMessage` to `AppScheduleProvider::getSchedule()`. Never create a second `#[AsSchedule]` provider.** A new `#[AsSchedule('foo')]` creates a matching `scheduler_foo` transport that is silently registered but never fires unless the worker command is updated to consume it — that class of bug wiped out five recurring jobs in production. The guard test `tests/Unit/Scheduler/ScheduleProviderConventionTest.php` fails the build if a second provider is introduced.

Every recurring message must:
- Route to an async transport (usually `async`) in `config/packages/messenger.yaml` — the handler runs in the async consumer, **not** inside the scheduler worker loop.
- Use `every('1 hour')` for interval jobs or `cron('...')` for time-of-day jobs. `#midnight` is hashed across hosts to avoid thundering-herd on multi-replica setups.

The scheduler worker must run **exactly one replica** — Symfony Scheduler has no built-in distributed lock, and two replicas will double-fire every job (duplicate billing, duplicate expirations). The single-replica constraint is enforced by compose (`messenger_worker` has no `deploy.replicas` override) and called out in the compose file comments.
